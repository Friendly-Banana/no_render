 ...
 *
 * For the former one, use the 3-argument constructor {@link #ConfigurationScreen(Screen, TriFunction)} and return your own screen from the TriFunction. For the latter,
 * use either the 2-argument constructor {@link #ConfigurationScreen(Screen)} if you don't need to extend the system, or the 3-argument one and return a subclass of
...
public final class ConfigurationScreen extends OptionsSubScreen {
    ...
    public static class TranslationChecker {
        ...
        public void finish() {
            ...
            if (CommonAbstractions.INSTANCE.isDevelopmentEnvironment() && (!untranslatables.isEmpty() || !untranslatablesWithFallback.isEmpty())) {
    ...
    // Ideally this should not be static, but we need it in the constructor's super() call
    ...
    public ConfigurationScreen(final Screen parent) {
        this(parent, ConfigurationSectionScreen::new);
    }

    public ConfigurationScreen(final Screen parent, ConfigurationSectionScreen.Filter filter) {
        this(parent, (a, b, c, d) -> new ConfigurationSectionScreen(a, b, c, d, filter));
    }

    @SuppressWarnings("resource")
    public ConfigurationScreen(final Screen parent, QuadFunction<ConfigurationScreen, ModConfig.Type, ModConfig, Component, Screen> sectionScreen) {
        super(parent, Minecraft.getInstance().options, Component.translatable(translationChecker.check(NoRender.MOD_ID + ".configuration.title", LANG_PREFIX + "title"), NoRender.MOD_NAME));
    ...
    public Component translatableConfig(ModConfig modConfig, String suffix, String fallback) {
        return Component.translatable(translationChecker.check(NoRender.MOD_ID + ".configuration.section." + modConfig.getFileName().replaceAll("[^a-zA-Z0-9]+", ".").replaceFirst("^\\.", "").replaceFirst("\\.$", "").toLowerCase(Locale.ENGLISH) + suffix, fallback), NoRender.MOD_NAME);
    }
     ...
     *
     * This class is automatically used if you use NeoForge's generic configuration UI, see {@link net.neoforged.neoforge.client.gui.ConfigurationScreen}.<p>
     *
     ...
     * <li>To use another UI element, override the matching <code>create*Value()</code> method and return your new UI element wrapped in a {@link ConfigurationSectionScreen.Element}.
    ...
    public static class ConfigurationSectionScreen extends OptionsSubScreen {
        ...
        public record Context(String modId, Screen parent, ModConfig modConfig, ModConfigSpec modSpec,
                Set<? extends Entry> entries, Map<String, Object> valueSpecs, List<String> keylist, ConfigurationSectionScreen.Filter filter) {
            @ApiStatus.Internal
            public Context {}

            public static ConfigurationSectionScreen.Context top(final String modId, final Screen parent, final ModConfig modConfig, ConfigurationSectionScreen.Filter filter) {
                return new ConfigurationSectionScreen.Context(modId, parent, modConfig, (ModConfigSpec) modConfig.getSpec(), ((ModConfigSpec) modConfig.getSpec()).getValues().entrySet(),
                        ((ModConfigSpec) modConfig.getSpec()).getSpec().valueMap(), List.of(), filter);
            }

            public static ConfigurationSectionScreen.Context section(final ConfigurationSectionScreen.Context parentContext, final Screen parent, final Set<? extends Entry> entries, final Map<String, Object> valueSpecs,
                                                                                                                           final String key) {
                return new ConfigurationSectionScreen.Context(parentContext.modId(), parent, parentContext.modConfig(), parentContext.modSpec(), entries, valueSpecs,
                        parentContext.makeKeyList(key), parentContext.filter);
            }

            public static ConfigurationSectionScreen.Context list(final ConfigurationSectionScreen.Context parentContext, final Screen parent) {
                return new ConfigurationSectionScreen.Context(parentContext.modId, parent, parentContext.modConfig, parentContext.modSpec,
        ...
        public interface Filter {
            @Nullable
            ConfigurationSectionScreen.Element filterEntry(ConfigurationSectionScreen.Context context, String key, ConfigurationSectionScreen.Element original);
        }

        protected final ConfigurationSectionScreen.Context context;
         ...
         *                  If this is a {@link net.neoforged.neoforge.client.gui.ConfigurationScreen}, additional information is passed before closing.
         ...
         *                  If this is a {@link net.neoforged.neoforge.client.gui.ConfigurationScreen}, additional information is passed before closing.
         * @param type      The {@link Type} this configuration is for. Only used to generate the title of the screen.
         * @param filter    The {@link ConfigurationSectionScreen.Filter} to use.
         * @param modConfig The actual config to show and edit.
         */
        public ConfigurationSectionScreen(final Screen parent, final ModConfig.Type type, final ModConfig modConfig, Component title, ConfigurationSectionScreen.Filter filter) {
            this(ConfigurationSectionScreen.Context.top(modConfig.getModId(), parent, modConfig, filter), title);
         ...
         *
         * @param parentContext The {@link ConfigurationSectionScreen.Context} object of the parent.
         * @param parent        The screen to return to when the user presses escape or the "Done" button.
         ...
         */
        public ConfigurationSectionScreen(final ConfigurationSectionScreen.Context parentContext, final Screen parent, final Map<String, Object> valueSpecs, final String key,
                                          final Set<? extends Entry> entrySet, Component title) {
            this(ConfigurationSectionScreen.Context.section(parentContext, parent, entrySet, valueSpecs, key), Component.translatable(CRUMB, parent.getTitle(), title));
        }

        @SuppressWarnings("resource")
        protected ConfigurationSectionScreen(final ConfigurationSectionScreen.Context context, final Component title) {
         ...
         *
         * @param key The key of the changed configuration. To get an absolute key, use {@link ConfigurationSectionScreen.Context#makeKeyList(String)}.
         */
        ...
        protected ConfigurationSectionScreen rebuild() {
            if (list != null) { // this may be called early, skip and wait for init() then
                ...
                final List<ConfigurationSectionScreen.@Nullable Element> elements = new ArrayList<>();
                ...
                for (final ConfigurationSectionScreen.Element element : elements) {
                    if (element != null) {
         ...
         *
         * @return A collection of {@link ConfigurationSectionScreen.Element}.
         */
        protected Collection<? extends ConfigurationSectionScreen.Element> createSyntheticValues() {
        ...
        @Nullable
        protected ConfigurationSectionScreen.Element createStringValue(final String key, final Predicate<String> tester, final Supplier<String> source, final Consumer<String> target) {
            if (source.get().length() > 192) {
                ...
                return new ConfigurationSectionScreen.Element(getTranslationComponent(key), getTooltipComponent(key, null), label, false);
            }
            ...
            return new ConfigurationSectionScreen.Element(getTranslationComponent(key), getTooltipComponent(key, null), box);
        }
         ...
         * @return null if no UI element should be added or an {@link ConfigurationSectionScreen.Element} to be added to the UI.
         */
        @Nullable
        protected ConfigurationSectionScreen.Element createOtherSection(final String key, final Object value) {
            return null;
        }
         ...
         */
        @Nullable
        protected ConfigurationSectionScreen.Element createOtherValue(final String key, final ConfigValue<?> value) {
        ...
        public record Custom<T>(List<T> values) implements OptionInstance.ValueSet<T> {
            ...
            public static final ConfigurationSectionScreen.Custom<Boolean> BOOLEAN_VALUES_NO_PREFIX = new ConfigurationSectionScreen.Custom<>(ImmutableList.of(Boolean.TRUE, Boolean.FALSE));
        }

        @Nullable
        protected ConfigurationSectionScreen.Element createBooleanValue(final String key, final ValueSpec spec, final Supplier<Boolean> source, final Consumer<Boolean> target) {
            return new ConfigurationSectionScreen.Element(getTranslationComponent(key), getTooltipComponent(key, null),
                    new OptionInstance<>(getTranslationKey(key), getTooltip(key, null), OptionInstance.BOOLEAN_TO_STRING, ConfigurationSectionScreen.Custom.BOOLEAN_VALUES_NO_PREFIX, source.get(), newValue -> {
        ...
        @Nullable
        protected <T extends Enum<T>> ConfigurationSectionScreen.Element createEnumValue(final String key, final ValueSpec spec, final Supplier<T> source, final Consumer<T> target) {
            ...
            return new ConfigurationSectionScreen.Element(getTranslationComponent(key), getTooltipComponent(key, null),
                    new OptionInstance<>(getTranslationKey(key), getTooltip(key, null), (caption, displayvalue) -> displayvalue instanceof TranslatableEnum tenum ? tenum.getTranslatedName() : Component.literal(displayvalue.name()),
                            new ConfigurationSectionScreen.Custom<>(list), source.get(), newValue -> {
                                // regarding change detection: new value always is different (cycle button)
                                undoManager.add(v -> {
                                    target.accept(v);
                                    onChanged(key);
        ...
        @Nullable
        protected ConfigurationSectionScreen.Element createIntegerValue(final String key, final ValueSpec spec, final Supplier<Integer> source, final Consumer<Integer> target) {
        ...
        @Nullable
        protected ConfigurationSectionScreen.Element createSlider(final String key, final Supplier<Integer> source, final Consumer<Integer> target, final @Nullable Range<Integer> range) {
            return new ConfigurationSectionScreen.Element(getTranslationComponent(key), getTooltipComponent(key, null),
        ...
        @Nullable
        protected ConfigurationSectionScreen.Element createLongValue(final String key, final ValueSpec spec, final Supplier<Long> source, final Consumer<Long> target) {
            return createNumberBox(key, spec, source, target, null, Long::decode, 0L);
        }

        // if someone knows how to get a proper zero inside...
        @Nullable
        protected <T extends Number & Comparable<? super T>> ConfigurationSectionScreen.Element createNumberBox(final String key, final ValueSpec spec, final Supplier<T> source,
        ...
        @Nullable
        protected ConfigurationSectionScreen.Element createDoubleValue(final String key, final ValueSpec spec, final Supplier<Double> source, final Consumer<Double> target) {
            return createNumberBox(key, spec, source, target, null, Double::parseDouble, 0.0);
        }

        @Nullable
        protected ConfigurationSectionScreen.Element createSection(final String key, final UnmodifiableConfig subconfig, final UnmodifiableConfig subsection) {
            if (subconfig.isEmpty()) return null;
            return new ConfigurationSectionScreen.Element(Component.translatable(SECTION, getTranslationComponent(key)), getTooltipComponent(key, null),
        ...
        @Nullable
        protected <T> ConfigurationSectionScreen.Element createList(final String key, final ListValueSpec spec, final ModConfigSpec.ConfigValue<List<T>> list) {
            return new ConfigurationSectionScreen.Element(Component.translatable(SECTION, getTranslationComponent(key)), getTooltipComponent(key, null),
                    Button.builder(Component.translatable(SECTION, Component.translatable(translationChecker.check(getTranslationKey(key) + ".button", SECTION_TEXT))),
                            button -> minecraft.setScreen(sectionCache.computeIfAbsent(key,
                                    k -> new ConfigurationListScreen<>(ConfigurationSectionScreen.Context.list(context, this), key, Component.translatable(CRUMB, this.getTitle(), getTranslationComponent(key)), spec, list)).rebuild()))
        ...
        public void onClose() {
            if (changed) {
                ...
                } else if (lastScreen instanceof final net.neoforged.neoforge.client.gui.ConfigurationScreen parent) {
                    parent.needsRestart = parent.needsRestart.with(needsRestart);
                }
            }
            super.onClose();
     ...
     * <li>To be notified on each changed value instead of getting one {@link ModConfigEvent} at the end, override {@link #onChanged(String)} on the {@link net.neoforged.neoforge.client.gui.ConfigurationScreen},
    ...
    public static class ConfigurationListScreen<T> extends ConfigurationSectionScreen {
        ...
        protected AbstractWidget createListLabel(int idx) {
            return new ConfigurationListScreen.ListLabelWidget(0, 0, Button.DEFAULT_WIDTH, Button.DEFAULT_HEIGHT, Component.translatable(LIST_ELEMENT, idx), idx);
        }
    ...
    public static final class UndoManager {
        ...
        public void add(final List<UndoManager.Step<?>> steps) {
            add(new UndoManager.Step<>(n -> steps.forEach(UndoManager.Step::runRedo), null, n -> steps.forEach(UndoManager.Step::runUndo), null), true);
        }
